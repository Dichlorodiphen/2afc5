...
controller = None
for c in steamvr.getControllerList():
...
models = {}
models['welcomeBox'] = viz.add('models/redBox.osgb')
models['viewPlate'] = viz.add('models/whiteBox.osgb')
...
def main():
	"""Run experiment, including orientation."""
	signal = viztask.Signal()
	viz.callback(viz.TIMER_EVENT, lambda n: masterLoop(n, signal))
	viz.starttimer(0,1/90,viz.FOREVER)
	
	# Initialization for experiment
	viztask.schedule(experiment.startExperiment(signal, controller))
